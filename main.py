...
@app.route('/fix-image-links', methods=['GET'])
def fix_image_links():
    # Connect to PostgreSQL
    conn = psycopg2.connect(
        host=os.environ["POSTGRESQL_DB_HOST"],
    ...
    # Get all products
    cur.execute("SELECT id, name, image_mongodb_id FROM products")
    products = cur.fetchall()
    ...
    # For each product with an image_mongodb_id, update MongoDB
    for product in products:
        product_id = product[0]
        product_name = product[1]
        image_mongodb_id = product[2]
        
        if image_mongodb_id:
            # Try to find the MongoDB document and update it with product_id
            try:
                from bson.objectid import ObjectId
                mongo_id = ObjectId(image_mongodb_id)
                result = collection.update_one(
                    {"_id": mongo_id},
                    {"$set": {"product_id": product_id}}
                )
                if result.modified_count > 0:
                    updates.append(f"Updated MongoDB image {image_mongodb_id} with product_id {product_id}")
            except Exception as e:
                updates.append(f"Error updating MongoDB: {str(e)}")
    
    # Now check for MongoDB images without product_id
    all_images = list(collection.find({"product_id": {"$exists": False}}))
    remaining_products = [p for p in products if not p[2]]  # Products without image_mongodb_id
    
    # Try to match remaining images to remaining products
    for i, img in enumerate(all_images):
        if i < len(remaining_products):
            product = remaining_products[i]
            product_id = product[0]
            
            # Update MongoDB with product_id
            collection.update_one(
                {"_id": img["_id"]},
                {"$set": {"product_id": product_id}}
            )
            ...
            cur.execute('''
                UPDATE products 
                SET image_mongodb_id = %s
                WHERE id = %s
            ''', (str(img["_id"]), product_id))
            
            updates.append(f"Linked product {product_id} to image {img.get('file_path', str(img['_id']))}")
    ...
    return jsonify({"message": "Image links fixed", "updates": updates})
